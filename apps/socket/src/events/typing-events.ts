 ...
 *
 * Handles:
 * - TYPING_START: User begins typing in a conversation
 * - TYPING_STOP: User stops typing
 *
 * Updates Redis for presence tracking and broadcasts to room members.
 *
...
function registerTypingEvents(socket: Socket) {
  ...
  /**
   * Handle TYPING_START - user is typing.
   * Updates Redis and broadcasts to conversation room.
   */
  socket.on(
    SOCKET_EVENTS.TYPING_START,
    async ({ conversationId, receiverId, type }) => {
      try {
